...
test "zero_struct_overhead" {
    try testing.expectEqual(@alignOf(@Vector(4, f32)), @alignOf(math.Quat));
    try testing.expectEqual(@sizeOf(@Vector(4, f32)), @sizeOf(math.Quat));
}

test "init" {
    try testing.expectEqual(math.Quat{
        .v = math.vec4(1, 2, 3, 4),
    }, math.quat(1, 2, 3, 4));
}
...
test "fromAxisAngle" {
    ...
    const actual = math.Quat.fromAxisAngle(math.vec3(1, 0, 0), math.pi / 4.0);
